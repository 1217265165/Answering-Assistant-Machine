...
以下是题目：\n\n`;
    ...
    // 模块 2：考试网页端逻辑 (双重绑定核心)
    // ==========================================
    let pickingType = null; // 可选值: 'question' (绑题目), 'nextBtn' (绑翻页), 'manual' (单次截图), null
    ...
    function generateUniqueSelector(el) {
        ...
        if (parentSelector) return `${parentSelector} > ${el.tagName.toLowerCase()}:nth-of-type(${getNthOfType(el)})`;
        return null;
    }
    ...
    function resetUiState() {
        ...
        const btnBindQ = document.getElementById('ds-btn-bind-q');
        const btnBindN = document.getElementById('ds-btn-bind-n');
        
        const savedQSelector = GM_getValue('ds_dynamic_selector', null);
        const savedNSelector = GM_getValue('ds_next_btn_selector', null);

        // 恢复按钮颜色
        if(btnBindQ) btnBindQ.style.background = '#f39c12';
        if(btnBindN) btnBindN.style.background = '#f39c12';

        if (savedQSelector) {
            if(statusLabel) {
                statusLabel.innerText = savedNSelector ? '🟢 题目与翻页均已绑定' : '🟡 题目已绑 / 翻页未绑';
            ...
            if(btn0) {
             ...
             if(statusLabel) {
                statusLabel.innerText = '⚪ 状态：尚未绑定题目';
                statusLabel.style.color = '#666';
            }
            if(btn0) {
                btn0.innerText = '⚠️ 请先绑定【题目框】';
    ...
    function createControlPanel() {
        ...
        statusRow.style.cssText = 'margin-bottom:2px; text-align:center;';
        ...
        bindGroup.style.cssText = 'display:flex; justify-content:space-between; gap:5px; margin-bottom:4px;';
        
        const btnBindQ = document.createElement('button');
        btnBindQ.id = 'ds-btn-bind-q';
        btnBindQ.innerText = '🎯 绑题目框';
        btnBindQ.style.cssText = 'flex:1; padding:6px; font-size:11px; background:#f39c12; color:white; border:none; border-radius:4px; cursor:pointer; font-weight:bold;';
        btnBindQ.addEventListener('click', (e) => {
            e.stopPropagation(); resetUiState(); pickingType = 'question';
            btnBindQ.innerText = '🖱️ 请点击题目'; btnBindQ.style.background = '#d35400';
            document.body.style.cursor = 'crosshair';
        });

        const btnBindN = document.createElement('button');
        btnBindN.id = 'ds-btn-bind-n';
        btnBindN.innerText = '⏭️ 绑翻页键';
        btnBindN.style.cssText = 'flex:1; padding:6px; font-size:11px; background:#f39c12; color:white; border:none; border-radius:4px; cursor:pointer; font-weight:bold;';
        btnBindN.title = '如果默认无法自动翻页触发，请用这个绑定你的“下一题”按钮';
        btnBindN.addEventListener('click', (e) => {
            e.stopPropagation(); resetUiState(); pickingType = 'nextBtn';
            btnBindN.innerText = '🖱️ 请点击下一题'; btnBindN.style.background = '#d35400';
            document.body.style.cursor = 'crosshair';
        });
        
        bindGroup.appendChild(btnBindQ);
        bindGroup.appendChild(btnBindN);
        ...
        promptInput.addEventListener('input', (e) => GM_setValue('ds_custom_prompt', e.target.value));
        ...
        btn0.addEventListener('click', (e) => {
            ...
            if (targetEl) processTargetElement(targetEl, btn0); 
            else alert(`⚠️ 找不到绑定的题目区域，请重新绑定。`); 
        });
        ...
        container.appendChild(bindGroup); // 新增的双重绑定按钮组
    ...
    document.addEventListener('mouseover', (e) => {
        if (!pickingType || e.target.closest('#ds-control-group')) return;
        if (lastHoveredElement && lastHoveredElement !== e.target) lastHoveredElement.style.outline = '';
        e.target.style.outline = (pickingType === 'nextBtn') ? '3px solid #f1c40f' : '3px solid red'; // 绑翻页时用黄框，绑题目用红框
    ...
    document.addEventListener('click', (e) => {
        ...
        if (pickingType === 'question' || pickingType === 'nextBtn') {
            e.preventDefault(); e.stopPropagation();
            const targetEl = e.target;
            const uniqueSelector = generateUniqueSelector(targetEl);
            
            if(uniqueSelector) {
                if (pickingType === 'question') {
                    GM_setValue('ds_dynamic_selector', uniqueSelector); 
                    console.log('✅ 题目绑定成功:', uniqueSelector);
                } else {
                    GM_setValue('ds_next_btn_selector', uniqueSelector); 
                    console.log('✅ 翻页按钮绑定成功:', uniqueSelector);
                }
                resetUiState(); 
            } else {
                alert('❌ 无法生成唯一路径，请换一个外层的框点击。');
            }
            return;
        }
    ...
    document.addEventListener('click', (e) => {
        ...
        // 1. 检查是否点击了用户绑定的翻页按钮
        const boundNextSelector = GM_getValue('ds_next_btn_selector', null);
        if (boundNextSelector && e.target.closest(boundNextSelector)) {
            isNextTriggered = true;
        }
        
        // 2. 智能兜底：就算没绑定，只要点击的按钮/链接文字里包含“下一题/下一页”，也自动触发
        const text = e.target.innerText?.trim() || '';
        if (!isNextTriggered && (text.includes('下一题') || text.includes('下一页') || text === '保存并下一题')) {
            isNextTriggered = true;
        }
        
        // 3. 智能兜底：答题卡上的数字题号点击触发
        if (!isNextTriggered && text.match(/^\d+$/) && e.target.clientWidth < 60) {
            isNextTriggered = true;
        }

        if (isNextTriggered) {
            console.log("⏭️ 捕获到翻页动作，等待 600ms 后盲传新题...");
            setTimeout(() => {
                const currentSelector = GM_getValue('ds_dynamic_selector', null);
                const targetEl = document.querySelector(currentSelector);
                const btn0 = document.getElementById('ds-btn-mode0');
